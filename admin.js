...
btnEntrarAdmin.addEventListener("click", async () => {
    ...
    try {
        ...
        if (resultado.user.email !== ADMIN_EMAIL) {
            await signOut(auth);
            mensagemAcesso.textContent = "Esta conta não possui acesso ao painel.";
        }
    } catch (erro) {
        console.error("Erro no login administrativo:", erro);
        mensagemAcesso.textContent = "Não foi possível entrar. Tente novamente.";
    } finally {
...
onAuthStateChanged(auth, async (usuario) => {
    ...
    document.getElementById("nomeAdmin").textContent = usuario.displayName || "Administrador";
...
async function carregarDados() {
    ...
    try {
        const [snapshotUsuarios, snapshotAnuncios] = await Promise.all([
            getDocs(collection(db, "usuarios")),
            getDocs(query(collection(db, "anuncios"), orderBy("criadoEm", "desc")))
        ]);
        ...
        alert("Não foi possível carregar os dados. Verifique as regras do Firestore.");
    } finally {
...
function atualizarResumo() {
    const ativos = anuncios.filter((anuncio) => obterStatus(anuncio) === "ativo").length;
    const creditos = profissionais.reduce((total, profissional) => {
...
function renderizarUltimosAnuncios() {
    ...
    if (ultimos.length === 0) {
        container.innerHTML = '<p class="estado-vazio">Nenhum anúncio cadastrado.</p>';
        return;
    }

    container.innerHTML = ultimos.map((anuncio) => {
        const status = obterStatus(anuncio);

        return `
            <div class="item-compacto">
                <div>
                    <strong>${escaparHTML(anuncio.titulo || "Sem título")}</strong>
                    <p>${escaparHTML(anuncio.nome || "Cliente não informado")} · ${escaparHTML(anuncio.categoria || "Sem categoria")}</p>
                </div>
                <span class="status ${status}">${status === "ativo" ? "Ativo" : "Expirado"}</span>
            </div>
        `;
    }).join("");
}
...
function renderizarProfissionais(lista) {
    ...
    tabela.innerHTML = lista.map((profissional) => `
        <tr>
            <td>${escaparHTML(profissional.nome || "Não informado")}</td>
            <td>${escaparHTML(profissional.email || "Não informado")}</td>
            <td>${escaparHTML(profissional.tipo || "profissional")}</td>
            <td>${Number(profissional.creditos || 0)}</td>
            <td>${formatarData(profissional.criadoEm)}</td>
        </tr>
    `).join("");
}
...
function renderizarAnuncios(lista) {
    ...
    const vazio = document.getElementById("vazioAnuncios");
    ...
    vazio.classList.add("oculto");
    container.innerHTML = lista.map((anuncio) => {
        const status = obterStatus(anuncio);

        return `
            <article class="anuncio-admin">
                <h3>${escaparHTML(anuncio.titulo || "Sem título")}</h3>
                <p><strong>Cliente:</strong> ${escaparHTML(anuncio.nome || "Não informado")}</p>
                <p><strong>Categoria:</strong> ${escaparHTML(anuncio.categoria || "Não informada")}</p>
                <p>${escaparHTML(anuncio.descricao || "Sem descrição")}</p>
                <div class="anuncio-rodape">
                    <span>${formatarData(anuncio.criadoEm)}</span>
                    <span class="status ${status}">${status === "ativo" ? "Ativo" : "Expirado"}</span>
                </div>
            </article>
        `;
...
function obterStatus(anuncio) {
    if (!anuncio.expiraEm || typeof anuncio.expiraEm.toDate !== "function") {
        return anuncio.status === "ativo" ? "ativo" : "expirado";
    }

    return anuncio.expiraEm.toDate().getTime() > Date.now() ? "ativo" : "expirado";
}
...
document.querySelectorAll(".menu-item").forEach((botao) => {
    botao.addEventListener("click", () => {
        ...
        document.getElementById("tituloSecao").textContent = botao.textContent;
    });
});

document.getElementById("buscaProfissional").addEventListener("input", (evento) => {
    ...
    const filtrados = profissionais.filter((profissional) => {
        ...
        const email = String(profissional.email || "").toLowerCase();
...
document.getElementById("buscaAnuncio").addEventListener("input", filtrarAnuncios);
document.getElementById("filtroStatus").addEventListener("change", filtrarAnuncios);

function filtrarAnuncios() {
    const termo = document.getElementById("buscaAnuncio").value.trim().toLowerCase();
    const statusSelecionado = document.getElementById("filtroStatus").value;

    const filtrados = anuncios.filter((anuncio) => {
        const texto = [anuncio.titulo, anuncio.nome, anuncio.categoria, anuncio.descricao]
            .map((item) => String(item || "").toLowerCase())
            .join(" ");

        const correspondeTexto = texto.includes(termo);
        const correspondeStatus = statusSelecionado === "todos" || obterStatus(anuncio) === statusSelecionado;
